...
(() => {
  const DEFAULT_CONFIG = {
    ...
    AUTO_SORT_ON_NEW_TAB: false,
    AUTO_SORT_DEBOUNCE_MS: 5000,
    SHOW_AUTO_SORT_TOGGLE: false,
  ...
  const PREFS = {
    ...
    AUTO_SORT_ON_NEW_TAB: ["bool", "behavior.auto-sort-on-new-tab"],
    AUTO_SORT_DEBOUNCE_MS: ["double", "behavior.auto-sort-debounce-ms"],
    SHOW_AUTO_SORT_TOGGLE: ["bool", "ui.show-auto-sort-toggle"],
  ...
  let autoSortTabOpenHandler = null;
...
- Return ONLY raw JSON. No markdown code fences, no prose, no explanations.`;
    ...
    const clickHandler = (event) => {
      ...
      switch (action) {
        ...
        case "collapse":
          ...
          break;
        case "auto-sort-toggle": {
          const newVal = !CONFIG.AUTO_SORT_ON_NEW_TAB;
          CONFIG.AUTO_SORT_ON_NEW_TAB = newVal;
          try {
            const prefName = `${PREF_BRANCH}behavior.auto-sort-on-new-tab`;
            services.prefs.setBoolPref(prefName, newVal);
          } catch (e) {
            console.warn("[TidyTabs] Could not persist auto-sort pref:", e);
          }
          // Update all auto-sort toggle buttons to reflect new state
          document.querySelectorAll('.tidy-tabs-inline-btn[data-action="auto-sort-toggle"]')
            .forEach((b) => restoreAutoSortToggleState(b));
          break;
        }
      }
    };
  ...
  // Sync the auto-sort toggle button's visual state to CONFIG.AUTO_SORT_ON_NEW_TAB
  const restoreAutoSortToggleState = (btn) => {
    if (!btn) return;
    const active = CONFIG.AUTO_SORT_ON_NEW_TAB;
    btn.classList.toggle("tidy-tabs-btn-active", active);
    btn.title = active ? "Auto-sort ON — click to disable" : "Auto-sort OFF — click to enable";
  ...
  const getInlineButtonDefs = () => {
    ...
    if (CONFIG.SHOW_AUTO_SORT_TOGGLE) {
      defs.push({
        action: "auto-sort-toggle",
        label: "Auto",
        icon: INLINE_ACTION_ICON_KEYS["auto-sort-toggle"],
        tooltip: "Toggle auto-sort on new tab",
      });
    }
    return defs;
  };
  ...
  const injectInlineButtons = () => {
    ...
    for (const sep of separators) {
      ...
      for (const def of buttonDefs) {
        ...
        // Restore auto-sort toggle state
        if (def.action === "auto-sort-toggle") {
          restoreAutoSortToggleState(btn);
        }
      }
  ...
  const TIDY_TABS_ICON_SVGS = {
    ...
    // auto-sort toggle (clock with arrow — "auto" feel)
    "auto-sort":
      `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="{{STROKE}}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 15"/></svg>`,
  };
